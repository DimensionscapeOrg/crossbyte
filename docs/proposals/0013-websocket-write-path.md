...
| ~~**Per-connection buffer metrics**~~ | Closed. `ServerWebSocket.publishMetrics()` and `HTTPServer`'s existing registry both publish `output_buffer_bytes_max` and `output_buffer_bytes_total` as aggregates across connections, walked on scrape. Nothing is labelled per peer. The max is measured rather than tracked as a running maximum, since a running maximum only rises and one stalled peer would pin it high forever. |
