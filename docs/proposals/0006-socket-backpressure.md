...
| Growth item | Notes |
|---|---|
| **Default limits on accepted connections** | `HTTPServer`/`ServerWebSocket` could apply a configured limit to every socket they accept, so applications get protection without touching each socket. |
