...
enum abstract IsolationLevel(String) to String {
     ...
     * by the server (e.g. `REPEATABLE-READ`) still map. Any other input is
     * rejected so an attacker-controlled string can never be concatenated into a
     * `SET ... ISOLATION LEVEL <v>` statement.
     */
    @:from public static function ofString(value:String):IsolationLevel {
        if (value == null) {
